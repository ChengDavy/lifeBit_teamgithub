...
//    self.headerView.backgroundColor = [UIColor redColor];
    self.headerLabel.text = [NSString stringWithFormat:@"%@老师今天有%lu节课",[HJUserManager shareInstance].user.pId,(unsigned long)self.currentClassArr.count];
...
-(void)getMessageConsulting{
    ...
    NSDictionary* parameters = @{
                                 ...
                                 @"baseUser.userid":[HJUserManager shareInstance].user.pId,
                                 @"searchVo.signer":[HJUserManager shareInstance].user.pId,
                                 @"baseUser.usertype":@"3"};
...
-(void)getVersionClick{
    ...
    NSDictionary *parameters = @{@"SCHOOL_ID":[HJUserManager shareInstance].user.pId};
    [self POST:KGet_Version parameters:parameters andSuccess:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        if ([[responseObject objectForKey:@"success"] intValue] == 1) {
            ...
            VersionModel *versionModel = [[LifeBitCoreDataManager shareInstance] efGetVersionModelWithTeacherId:[HJUserManager shareInstance].user.pId];
    ...
    [self POST:KGet_IpadCases parameters:parameters andSuccess:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            if ([[responseObject objectForKey:@"success"] boolValue]) {
                ...
                self.versionUploadCount--;
                [[LifeBitCoreDataManager shareInstance] efDeleteTearchAllWatchModel:[HJUserManager shareInstance].user.pId];
                ...
                if (deviceList.count > 0) {
                    ...
                    NSLog(@"devicesDicdevicesDic  %@",devicesDic);
                    ...
                    for (NSDictionary * dic in deviceArr) {
                        ...
                        watchModel.teacherId = [HJUserManager shareInstance].user.pId;
...
-(void)getLessonPlanProjectTypeList {
    ...
    [self POST:KProject_Type parameters:parameters andSuccess:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        ...
        if ([[responseObject objectForKey:@"status"] intValue] == 0) {
            ...
            NSString *namePath = [NSString stringWithFormat:@"feedback_v_%@.plist",[HJUserManager shareInstance].user.pId];
...
-(void)getScheduleList {
    ...
    self.manager.requestSerializer = [AFJSONRequestSerializer serializer];
    NSDictionary *parameters = @{ @"TEACHER_ID": [HJUserManager shareInstance].user.pId };
    ...
    [self POST:KGet_ScheduleList parameters:parameters andSuccess:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        ...
        if ([[responseObject objectForKey:@"success"] intValue] == 1) {
            ...
            NSMutableArray *haveClassArr = [[LifeBitCoreDataManager shareInstance] efGetAllHaveClassModel];
            self.versionUploadCount--;
            NSMutableArray *teacherScheduleArr = [[LifeBitCoreDataManager shareInstance] efGetClassAllScheduleModelWith:[HJUserManager shareInstance].user.pId];
            ...
            for (NSDictionary *scheduleDic in versionListArr) {
                ...
                scheduleModel.teacherId = [HJUserManager shareInstance].user.pId;
...
-(void)getTestProjectList {
    ...
    [self POST:KGet_TestProject parameters:parameters andSuccess:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        ...
        if ([[responseObject objectForKey:@"success"] intValue] == 1) {
            ...
            NSString* documentPath = [NSString GetDocumentsPath];
            NSString *namePath = [NSString stringWithFormat:@"project_v_%@.plist",[HJUserManager shareInstance].user.pId];
            NSString* path1 = [documentPath stringByAppendingPathComponent:namePath];
//             去除null
...
-(void)getSchoolGreadList {
    ...
    [self POST:KGet_SchoolGread parameters:parameters andSuccess:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        ...
        if ([[responseObject objectForKey:@"success"] intValue] == 1) {
            ...
            NSString* documentPath = [NSString GetDocumentsPath];
            NSString *namePath = [NSString stringWithFormat:@"grade_v_%@.plist",[HJUserManager shareInstance].user.pId];
            NSString* path1 = [documentPath stringByAppendingPathComponent:namePath];
...
-(void)getMyLessonPlan {
    ...
    NSDictionary* parameters = @{ @"teacherId": [HJUserManager shareInstance].user.pId } ;
...
-(void)getLessonPlanList{
    NSDictionary* parameters = @{ @"type":@(2), 
                                  ...
                                  @"currentPage":@(0),
                                  @"pageSize":@(30),
                                  @"referId":[HJUserManager shareInstance].user.pId } ;
