...
//获取随机字符串
+ (NSString* )getStrArc4randomWithSize:(NSInteger )size
{
//    int NUMBER_OF_CHARS = 10;
    char data[size];
    for (int x=0;x<size;data[x++] = (char)('A' + (arc4random_uniform(26))));
    
    NSString *dataPoint = [[NSString alloc] initWithBytes:data length:size encoding:NSUTF8StringEncoding];
...
+ (NSString*)getTimeFromSecondStr:(NSString *)Second{
    
    
    int timeRun = [Second intValue];
