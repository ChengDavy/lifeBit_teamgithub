...
-(BOOL)efIsLogin{
    ...
    NSLog(@"self.user.pPass   %@",self.user.pPass);
    
    return self.user.pPass != nil;
...
{
    NSString *path = [[NSString GetDocumentsPath] stringByAppendingPathComponent:(NSString*)c_my_account_archive_file_name];
    CUserModel *tmpUser =  (CUserModel *)[NSKeyedUnarchiver unarchiveObjectWithFile:path];
    NSLog(@"%@",tmpUser);
    self.user = tmpUser;
}
